...
- CF Worker generated HTML: NEVER DOMContentLoaded or IIFE wrappers → scopes functions away from onclick= (ReferenceError: Can't find variable); use function declarations at top of <script> placed at end of <body>
...
- expo-router modals: router.dismiss() not router.back() (back navigates, dismiss closes); router.dismissAll() not router.navigate() when clearing a full multi-modal stack after install
