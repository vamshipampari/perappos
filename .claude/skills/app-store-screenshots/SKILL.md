...
**Screenshots are advertisements, not documentation.** Every screenshot sells one idea. If you're showing UI, you're doing it wrong — you're selling a _feeling_, an _outcome_, or killing a _pain point_.
...
{
  LOCALES.map((l) => (
    <button key={l} onClick={() => setLocale(l)} style={{ fontWeight: locale === l ? 700 : 400 }}>
      {l.toUpperCase()}
    </button>
  ));
}
